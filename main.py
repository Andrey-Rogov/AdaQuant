...
def main_worker(args):
    ...
    if (args.absorb_bn or args.load_from_vision or args.pretrained) and not args.batch_norn_tuning:
        if args.load_from_vision:
            import torchvision
            exec_lfv_str = 'torchvision.models.' + args.load_from_vision + '(weights=ResNet50_Weights.IMAGENET1K_V2)'
            model = eval(exec_lfv_str)
            if 'pytcv' in args.model:
                from pytorchcv.model_provider import get_model as ptcv_get_model
                exec_lfv_str = 'ptcv_get_model("' + args.load_from_vision + '", weights=ResNet50_Weights.IMAGENET1K_V2)'
                model = eval(exec_lfv_str)
